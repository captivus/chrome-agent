...
Install: `uv tool install chrome-agent` (or `pip install chrome-agent`). Requires Google Chrome or Chromium (`--chrome-path` or `CHROME_AGENT_PATH` names one outside the standard locations). One runtime dependency (`websockets`); no Playwright, no browser downloads.
