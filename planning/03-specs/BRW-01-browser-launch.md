...
Binary discovery has four steps, most explicit first: the `--chrome-path` flag, then `CHROME_AGENT_PATH`, then the platform-specific standard paths (on Linux google-chrome, chromium-browser and chromium in standard locations; on macOS the Applications directory; on Windows Program Files), then a PATH search for the usual command names (google-chrome, google-chrome-stable, chromium-browser, chromium, chrome). If nothing is found it fails with an error listing what it searched, naming CHROME_AGENT_PATH, --chrome-path and PATH as the fixes.

PATH comes after the standard paths, not before, so it only fires where discovery used to fail outright. That matters on macOS, where the standard location is never on PATH: a machine with both `Google Chrome.app` and a Homebrew `chromium` has to keep resolving the former.

Neither override falls back. A value that names no executable fails the launch, reporting it and its source, instead of starting a different browser -- both forms were set for chrome-agent, so a bad one is an error. The variable is `CHROME_AGENT_PATH`, not `CHROME_PATH`: that name is Lighthouse's chrome-launcher's and is never read here, since another tool's value should not pick the browser on a host that already resolves one.

The overrides and the PATH step exist because the standard paths are unreachable on a growing number of hosts: a Playwright chromium under `~/.cache/ms-playwright/`, a Nix store path, a container where `/usr/bin` cannot be written. Before them, such a host could not launch at all.
...
resolve_executable(path):
    ...
    // searching PATH, so one call takes an absolute path or a bare command
    // name. "~" is expanded -- a container env file or unit file never is.
    return which(expanduser(path))

override_in_effect(chrome_path=None):
    // The override in play, and how it was given -- the second half is
    // what the not-found message names.
    if chrome_path:
...
searched_locations():
    // What the not-found error reports when there is no override. With one,
    // nothing else is searched, so the message names it instead.
